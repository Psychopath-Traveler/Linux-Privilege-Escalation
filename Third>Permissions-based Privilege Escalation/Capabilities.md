...
### Set Capability
```
victim@htb[/htb]$ sudo setcap cap_net_bind_service=+ep /usr/bin/vim.basic
```
...
### Enumerating Capabilities
```
victim@htb[/htb]$ find /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin -type f -exec getcap {} \;
...
```
victim@htb[/htb]$ getcap /usr/bin/vim.basic
