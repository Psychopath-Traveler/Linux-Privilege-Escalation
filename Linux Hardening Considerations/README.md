...
1. Audit writable files and directories and any binaries set with the SUID bit.
2. Ensure that any cron jobs and sudo privileges specify any binaries using the absolute path.
3. Do not store credentials in cleartext in world-readable files.
4. Clean up home directories and bash history.
5. Ensure that low-privileged users cannot modify any custom libraries called by programs.
6. Remove any unnecessary packages and services that potentially increase the attack surface.
7. Consider implementing **[SELinux](https://www.redhat.com/en/topics/linux/what-is-selinux)**, which provides additional access controls on the system.
...
Templates exist for configuration management automation tools such as **[Puppet](https://www.puppet.com/why-puppet/use-cases/continuous-configuration-automation)**, **[SaltStack](https://github.com/saltstack/salt)**, **[Zabbix](https://en.wikipedia.org/wiki/Zabbix)** and **[Nagios](https://en.wikipedia.org/wiki/Nagios)** to automate such checks and can be used to push messages to a Slack channel or email box as well as via other methods. Remote actions (Zabbix) and Remediation Actions (Nagios) can be used to find and auto correct these issues over a fleet of nodes. Tools such as Zabbix also feature functions such as checksum verification, which can be used for both version control and to confirm sensitive binaries have not been tampered with. For example, via the **[vfs.file.cksum](https://www.zabbix.com/documentation/4.0/en/manual/config/items/itemtypes/zabbix_agent)** file.

# Audit

Perform periodic security and configuration checks of all systems. There are several security baselines such as the DISA **[Security Technical Implementation Guides (STIGs)](https://public.cyber.mil/stigs/)** that can be followed to set a standard for security across all operating system types and devices. Many compliance frameworks exist, such as **[ISO27001](https://www.iso.org/standard/27001)**, **[PCI-DSS](https://www.pcisecuritystandards.org/about_us/)**, and **[HIPAA](https://www.hhs.gov/hipaa/for-professionals/security/index.html)** which can be used by an organization to help establish security baselines. These should all be used as reference guides and not the basis for a security program. A strong security program should have controls tailored to the organization's needs, operating environment, and the types of data that they store and process (i.e., personal health information, financial data, trade secrets, or publicly available information).
